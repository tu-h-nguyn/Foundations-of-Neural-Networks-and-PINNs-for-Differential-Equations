...
\item[TN11] \alert{bài toán ngược, PINN so với liên hợp rời rạc}
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\smallskip
\alert{Không động lực nào có mặt} trong TN1--TN10: thuận,
...
%==============================================================
\begin{frame}{RAD --- lấy mẫu lại toàn bộ: dự đoán của báo cáo sai}
\centering
\includegraphics[width=0.7\textwidth]{../Images/chap_5/fig59_tn10c.pdf}

\vspace{-2pt}
\begin{columns}[T]
\begin{column}{0.44\textwidth}
\footnotesize
Giả thuyết ghi \alert{trước khi chạy}:\\
H1 RAD thắng ``không thêm'' về $\varepsilon_{L^2}$: \bad{không đạt} (2/5)\\
H2 RAD không làm vùng trơn tệ đi: \bad{không đạt} (0/5)\\
H3 hạng $c$ bảo vệ vùng trơn: \good{đạt} (4/5)
\end{column}
\begin{column}{0.53\textwidth}
\footnotesize
\textbf{Kết luận:} cả 3 chiến lược thích nghi làm vùng trơn tệ đi ở
\alert{15/15} lần so sánh. Ở cấu hình này, cách đặt điểm chỉ
\emph{phân phối lại} sai số, không giảm tổng sai số.
\end{column}
\end{columns}
\end{frame}
...
%==============================================================
\begin{frame}{TN11: bài toán ngược --- đặt PINN vào đúng động lực của nó}
\footnotesize
Tìm $\zeta_1, \zeta_2$ trong $u_t + \zeta_1 u u_x - \zeta_2 u_{xx} = 0$ từ
$2\,000$ điểm đo, \alert{không biết điều kiện đầu và biên}. Đối thủ cổ điển được
cho \emph{cùng} thông tin: sai phân + liên hợp rời rạc viết tay, đầu/biên là ẩn.

\smallskip
\begin{tabular}{@{}lccc@{}}
\toprule
& sai số $\zeta_2$, sạch & sai số $\zeta_2$, nhiễu 1\% & giây tới khi $\zeta_2$ ổn định (sạch / nhiễu) \\
\midrule
PINN & \good{$0{,}29\%$} & $0{,}49\%$ & $109$ / $142$ \\
Liên hợp, $N_x = 1\,023$ & $0{,}89\%$ & \good{$0{,}38\%$} & \good{$43$ / $34$} \\
Cổ điển, \emph{biết} đầu/biên & $0{,}82\%$ & $0{,}41\%$ & $17$ / $9$ \\
\bottomrule
\end{tabular}
...
\scriptsize
Giả thuyết ghi \alert{trước khi chạy}:\\
H1 PINN sạch $< 1\%$ ở $5/5$: \bad{không} ($1{,}81\%$ ở một hạt)\\
H2 PINN nhiễu $< 5\%$: \good{đạt}\\
H3 ``$\zeta_2$ luôn sai hơn $\zeta_1$ một bậc'': \bad{không} ($8/10$)\\
H4 PINN chính xác hơn / nhanh hơn: \bad{không} / \good{đạt}${}^{*}$
\end{column}
\begin{column}{0.5\textwidth}
\begin{alertblock}{Rẻ để viết, không rẻ để chạy}
\footnotesize PINN: \good{7 dòng mã}. Liên hợp: \bad{133 dòng}, kiểm gradient,
chặn ổn định. Nhưng khi đã có, nó chính xác ngang và có đáp số sớm hơn.
\end{alertblock}
\end{column}
\end{columns}

\vspace{2pt}
{\scriptsize ${}^{*}$ đạt theo thời gian tới khi dừng; theo lúc $\zeta_2$ ổn định
thì liên hợp nhanh hơn $2{,}5$--$4{,}2$ lần.}
\end{frame}

%==============================================================
\begin{frame}{Xem nó xảy ra: PINN tìm độ nhớt từ 2\,000 điểm đo}
\centering
\hoathinh[0.68\textheight]{tn11_nhan_dang}

\footnotesize Hạt giống trung vị, nhiễu 1\%. $\zeta_1$ từ $0$ lên $1$; lớp sốc
hiện ra từ các chấm đo rời rạc.
\end{frame}

%==============================================================
\begin{frame}{Đối chiếu lý thuyết và thực nghiệm: 16 hạng mục}
\begin{columns}[T]
\begin{column}{0.46\textwidth}
\small
\begin{tabular}{@{}lr@{}}
...
\smallskip
Cả chín hạng mục phải điều chỉnh đều đến từ bốn kiểu \emph{suy luận yếu hơn}.
\end{alertblock}
\end{column}
\end{columns}
\end{frame}
...
\item Khái quát hoá từ \emph{một lần chạy}
  ...
  ``sai số $\zeta_2$ \emph{luôn} lớn hơn $\zeta_1$ một bậc'' (đúng $8/10$).}
\item Phát biểu định lượng hay so sánh chưa từng được đo
  \\{\footnotesize Nhận xét 3.15 đoán $4$--$6$, đo được $\approx 9$; ``ưu thế nổi
  bật'' của PINN ở bài toán ngược.}
\end{enumerate}

\medskip
\centering
\alert{Không kiểu nào bị phát hiện bởi một lần chạy.}\\
Ba kiểu đầu cần nhiều hạt giống; kiểu thứ tư cần một phép đo, hoặc một đối thủ.
\end{frame}
...
\small
\begin{itemize}
\item \textbf{Phạm vi hẹp.} Gần như chỉ bài toán thuận (một bài toán ngược, hai
  tham số), một chiều, mạng truyền thẳng.
...
\item \textbf{Chỉ một trong ba động lực của Chương 1 được thử} (TN11, bài toán
  ngược). TN1--TN10 kiểm chứng \emph{cơ chế}, không kiểm chứng
...
Mười một thí nghiệm với tiêu chí đặt trước. Kết quả tiêu cực được báo cáo đầy đủ ---
năm trong số đó buộc phải siết lại chính các chương lý thuyết, và một dẫn tới
một bổ đề mới.
\end{block}
